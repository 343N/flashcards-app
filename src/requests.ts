...
class RequestHandler {
  ...
  public doDefaultRequest(
    ...
    if (req.method == undefined)
      return DEFAULT_REQUESTS["*"](req, res, options);
    if (req.method in DEFAULT_REQUESTS)
      return DEFAULT_REQUESTS[req.method](req, res, options);
  }

  public handleRequest(req: Request, res: Response): void {
    if (req.method == undefined)
      return this.doDefaultRequest(req, res, new HandlerOptions());
...
const DEFAULT_REQUESTS: any = {
  ...
  GET: (req: Request, res: Response, options: HandlerOptions): void => {
    ...
    if (fs.existsSync(filepath)) {
      ...
      res.setHeader(
        "Content-Type",
        mime.contentType(path.extname(filepath)) as string
      );
      res.writeHead(200);
...
const PUT_REQUESTS: any = {
  "/addCard": async (req: Request, res: Response, options: HandlerOptions) => {
    ...
    let curSess = sm.getSessionFromRequest(req);
    let cardAdded;
    if (curSess) cardAdded = sm.addSessionCard(curSess, new Card(arr));
  ...
  "/editCard": async (req: Request, res: Response, options: HandlerOptions) => {
    ...
    if (curSess && cards[data.id]) {
      ...
      if (front.trim().length != 0) cards[data.id].front = front;
      if (back.trim().length != 0) cards[data.id].back = back;
